...
Pre-1.0 (`v0.0.1`), active development. Core solver infrastructure and all
...
For pip-style editable installation in an existing environment:
...
> **Note:** The examples below use paths relative to the repository root
> (e.g. `"prometheus/thermo_data/nasa7.json"`).  Run Python from the repo
> root, or replace these with absolute paths for your environment.
...
```python
from prometheus.equilibrium import SpeciesDatabase, PerformanceSolver
from prometheus.propellants import PropellantDatabase
...
uv run black prometheus tests
uv run isort prometheus tests
