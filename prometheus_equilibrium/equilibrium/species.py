...
from prometheus_equilibrium.core.constants import ELEMENTS_MOLAR_MASSES
from prometheus_equilibrium.core.constants import UNIVERSAL_GAS_CONSTANT as R

try:
    from prometheus_equilibrium.equilibrium._thermo_kernels import (
        _cp_afcesic,
        _cp_n7,
        _cp_n9,
        _cp_shomate,
...
class SpeciesDatabase:
    ...
    @staticmethod
    def _default_thermo_path(filename: str) -> str:
        """Return the package-relative default path for a thermo data file."""
        thermo_dir = Path(__file__).resolve().parents[1] / "thermo_data"
        return str(thermo_dir / filename)
    ...
    def __init__(
        ...
        # Resolve defaults only when the caller leaves a path unspecified.
        self.nasa7_path = (
            nasa7_path
            if nasa7_path is not None
            else self._default_thermo_path("nasa7.json")
        )
        self.nasa9_path = (
            nasa9_path
            if nasa9_path is not None
            else self._default_thermo_path("nasa9.json")
        )
        self.janaf_path = (
            janaf_path
            if janaf_path is not None
            else self._default_thermo_path("janaf.csv")
        )
        self.shomate_path = (
            shomate_path
            if shomate_path is not None
            else self._default_thermo_path("shomate.json")
        )
        self.afcesic_path = (
            afcesic_path
            if afcesic_path is not None
            else self._default_thermo_path("afcesic.json")
        )
        self.terra_path = (
            terra_path
            if terra_path is not None
            else self._default_thermo_path("terra.json")
        )
    ...
    def _build_priority_map(
        ...
        default_map = {
            src: len(default_order) - i for i, src in enumerate(default_order)
        }
    ...
    def load(
        ...
        include_afcesic: bool = False,
        include_janaf: bool = False,
        include_shomate: bool = False,
    ...
    def _load_afcesic(self) -> None:
        ...
        for sp_id, rec in data.items():
            if sp_id.startswith("_"):
                continue
            if rec.get("calibration_source") == "TERRA_STOICH" and rec.get("phase") in (
                "S",
                "L",
            ):
                continue
