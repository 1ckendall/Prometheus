...
from prometheus.core.constants import REFERENCE_PRESSURE as _P_REF
from prometheus.core.constants import UNIVERSAL_GAS_CONSTANT as _R
from prometheus.equilibrium.element_matrix import ElementMatrix
from prometheus.equilibrium.mixture import Mixture
from prometheus.equilibrium.problem import EquilibriumProblem, ProblemType
from prometheus.equilibrium.solution import ConvergenceStep, EquilibriumSolution
...
class EquilibriumSolver(ABC):
    ...
    @staticmethod
    def _history_snapshot(mixture: Mixture, n_gas_total: Optional[float] = None) -> dict:
...
class MajorSpeciesSolver(_ReactionAdjustmentBase):
    ...
    * **PEP outer temperature search** with interval-halving fallback.
    ...
    def _update_minor_from_potentials(
        ...
        gas_idx = idx[np.array([mixture.species[i].condensed == 0 for i in idx], dtype=bool)]
        if gas_idx.size == 0:
            return

        g0 = np.array([mixture.species[i].reduced_gibbs(T) for i in gas_idx], dtype=float)
...
class GordonMcBrideSolver(EquilibriumSolver):
    ...
    * **Include**: any inactive condensed with μ°c/RT < Σₖ πₖ·aₖc is a
    ...
    def solve(
        ...
        species_pool = [
            sp for sp in problem.products if all(el in active_set for el in sp.elements.keys())
        ]
        initial_moles = np.array(
            [
                next((float(n) for s, n in zip(base_mixture.species, base_mixture.moles) if s is sp), 0.0)
        ...
        T = problem.constraint1 if problem.problem_type.fixed_temperature else problem.t_init
    ...
    def _refresh_thermo_species_set(
        ...
        if n_gas == 0:
            raise RuntimeError(
                f"No gas-phase species are thermo-valid at T={T:.2f} K."
            )
