...
from prometheus_equilibrium.thermo_data.parsers._common import normalise_element
...
def _assign_roles(name: str, elements: Dict[str, float], flags: str) -> List[str]:
    ...
    if (
        oxidizer_strength >= 0.45
        or has_oxidizer_hint
        or oxidizer_supply >= 0.95 * reducing_demand
    ):
    ...
    if metal_atoms / total_atoms >= 0.55 or upper_name.startswith(
