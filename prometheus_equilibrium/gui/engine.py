...
class EngineDock(QDockWidget):
    ...
    def on_calculate(self):
        ...
        if not jobs:
            self.main_window.statusBar().showMessage("No valid simulation cases generated.", 5000)
            return
        ...
        if sim_page.sim_tabs.currentIndex() == 0:
            ...
            sweep_axis = "pc" if self.pc_mode_combo.currentText() == "Sweep Range" else "none"
    ...
    def on_perf_finished(self, result):
        ...
        if shifting.chamber.converged and frozen.chamber.converged:
            ...
            species_moles = list(
                zip(chamber.mixture.species, chamber.mixture.moles)
            )
