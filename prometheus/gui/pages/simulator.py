...
from prometheus.gui.dialogs.database_search import DatabaseSearchDialog
...
class SimulatorPage(QWidget):
    ...
    def setup_bipropellant_tab(self):
        ...
        # Build tables
        self.fuel_table, self.fuel_tot, self.fuel_density_lbl = self._build_biprop_table(
        ...
        self.input_of_ratio.valueChanged.connect(lambda _: self._update_biprop_combined_density())
    ...
    def _update_biprop_combined_density(self):
        ...
        if not hasattr(self, "input_of_ratio") or not hasattr(self, "lbl_combined_density"):
            return
    ...
    def _update_biprop_totals(self, table, tot_disp, density_lbl):
        ...
        density_lbl.setText(f"ρ: {rho / 1000:.3f} g/cc" if rho is not None else "ρ: N/A")
    ...
    def update_fuel_totals(self):
        self._update_biprop_totals(self.fuel_table, self.fuel_tot, self.fuel_density_lbl)
    ...
    def apply_composition_snapshot(self, payload):
        ...
        if propellant_type not in ("bipropellant", "solid"):
            raise ValueError("Composition propellant_type must be 'bipropellant' or 'solid'.")
        ...
        if propellant_type == "bipropellant":
            ...
            self._load_rows(self.fuel_table, self.add_fuel_row, components.get("fuel", []))
            self._load_rows(self.ox_table, self.add_ox_row, components.get("oxidizer", []))
