...
from prometheus.core.constants import REFERENCE_PRESSURE as P_REF
...
from prometheus.equilibrium.species import Species
...
class _WindowedConstGibbsGas(_ConstGibbsGas):
    ...
    def __init__(
        ...
        super().__init__(elements, g0_RT=g0_RT, h0_RT=h0_RT, molar_mass_kg=molar_mass_kg)
...
def test_hp_reintroduces_species_when_temperature_cools():
    ...
    idx_cool = next((i for i, sp in enumerate(sol.mixture.species) if sp is sp_cool), None)
...
def test_sp_nozzle_style_reintroduces_species_on_cooling():
    ...
    """
    sp_X = _LogEntropyGas({"X": 1}, cp_over_r=4.0, s_ref_over_r=2.0, molar_mass_kg=0.001)
