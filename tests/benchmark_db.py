...
def _print_janaf_coverage(databases: Dict[str, SpeciesDatabase]) -> None:
    ...
    * Before the REF-phase fix, H2 / O2 / N2 were stored as H2_REF / O2_REF / N2_REF
    ...
    print(
        f"  {'Species':<10}  {'T [K]':>8}  {'NASA-9':>12}  {'JANAF':>12}  {'diff':>10}"
    )
    print(f"  {'-'*60}")
    for formula, temps in [
        ("OH", [298.15, 1000.0, 3000.0]),
        ("H2O", [298.15, 1000.0, 3000.0]),
    ]:
        try:
        ...
        for T in temps:
            ...
            h_j = sp_j.enthalpy(T)
            print(
                f"  {formula:<10}  {T:>8.2f}  {h_n:>12.1f}  {h_j:>12.1f}  {h_j-h_n:>10.1f}"
            )
