...
uv run python prometheus/thermo_data/parsers/cea.py           # Re-convert CEA thermo.inp → cea_nasa9.json
...
uv run black prometheus/ tests/                               # Format code
uv run isort prometheus/ tests/                               # Sort imports
```
...
db = SpeciesDatabase(
    ...
    cea_nasa9_path="prometheus/thermo_data/cea_nasa9.json",  # optional, CEA-sourced polynomials
)
...
The docs use `sphinx.ext.autodoc` + `sphinx.ext.autosummary` (with `:recursive:`) to automatically discover and document every module, class, and function in `prometheus/`. The Napoleon extension translates Google-style docstrings. Theme: `renku` (package: `renku-sphinx-theme`).
